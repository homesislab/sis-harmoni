...
class Vehicle_model extends CI_Model
{
    ...
    public function validate_payload(array $in, bool $is_create): array
    {
        ...
        if (isset($in['type'])) {
            ...
            if (!in_array($t, ['motor','mobil','lainnya'], true)) $err['type'] = 'Nilai tidak valid';
        }
    ...
    public function update(int $id, array $data): void
    {
        ...
        if ($upd) $this->db->where('id', $id)->update('vehicles', $upd);
    }
    ...
    public function paginate_with_household(int $page, int $per, ?int $person_id = null): array
    {
        ...
        if ($person_id) $this->db->where('v.person_id', $person_id);
