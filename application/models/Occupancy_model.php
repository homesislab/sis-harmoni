...
class Occupancy_model extends CI_Model
{
...
public function update(int $id, array $data): void
    ...
    public function paginate(int $page, int $per, array $filters = []): array
    {
        ...
        if (!empty($filters['house_id'])) $qb->where('house_id', (int)$filters['house_id']);
        if (!empty($filters['household_id'])) $qb->where('household_id', (int)$filters['household_id']);
        if (!empty($filters['status'])) $qb->where('status', (string)$filters['status']);
        if (!empty($filters['occupancy_type'])) $qb->where('occupancy_type', (string)$filters['occupancy_type']);
